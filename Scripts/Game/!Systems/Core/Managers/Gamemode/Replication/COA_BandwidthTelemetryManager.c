...
class COA_RPCTelemetryData
{
	...
	//------------------------------------------------------------------------------------------------
	void COA_RPCTelemetryData(string rpcName, int bytes)
	{
		m_sRPCName = rpcName;
		m_iCallCount = 1;
		m_iTotalBytes = bytes;
...
//------------------------------------------------------------------------------------------------
class COA_BandwidthTelemetryManager : SCR_BaseGameModeComponent
{
//=============================================================================================================================================================================================================================================================================================================================================================
//	 ATTRIBUTES
//=============================================================================================================================================================================================================================================================================================================================================================
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	protected ref map<string, ref COA_RPCTelemetryData> m_mTelemetryData = new map<string, ref COA_RPCTelemetryData>();
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	override void OnPostInit(IEntity owner)
	{
		...
		{
			Print("[COA_BandwidthTelemetry] Telemetry system initialized", LogLevel.NORMAL);
			Print(string.Format("[COA_BandwidthTelemetry] Summary interval: %1 seconds", m_iSummaryInterval), LogLevel.NORMAL);
			Print(string.Format("[COA_BandwidthTelemetry] Verbose logging: %1", m_bLogIndividualCalls), LogLevel.NORMAL);
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	protected void PrintTelemetrySummary()
	{
		...
		{
			Print("[COA_BandwidthTelemetry] No RPC calls recorded in this interval", LogLevel.NORMAL);
			return;
		}
		
		Print("========================================", LogLevel.NORMAL);
		Print("[COA_BandwidthTelemetry] RPC Bandwidth Summary", LogLevel.NORMAL);
		Print("========================================", LogLevel.NORMAL);
		...
		array<ref COA_RPCTelemetryData> sortedData = new array<ref COA_RPCTelemetryData>();
		foreach (string rpcName, COA_RPCTelemetryData data : m_mTelemetryData)
		{
		...
		for (int i = 0; i < sortedData.Count() - 1; i++)
		{
			for (int j = 0; j < sortedData.Count() - i - 1; j++)
			{
				...
				{
					COA_RPCTelemetryData temp = sortedData[j];
		...
		{
			COA_RPCTelemetryData data = sortedData[i];
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	void LogRPC(string rpcName, int estimatedBytes)
	{
		...
		{
			Print(string.Format("[COA_BandwidthTelemetry] RPC: %1 | Size: %2 bytes", rpcName, estimatedBytes), LogLevel.VERBOSE);
		}
		
		// Update telemetry data
		COA_RPCTelemetryData data = m_mTelemetryData.Get(rpcName);
		if (!data)
		{
			data = new COA_RPCTelemetryData(rpcName, estimatedBytes);
	...
	void ResetTelemetry()
	{
		...
		Print("[COA_BandwidthTelemetry] Telemetry data reset", LogLevel.NORMAL);
	}
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	//------------------------------------------------------------------------------------------------
	protected static COA_BandwidthTelemetryManager m_sInstance;
	void COA_BandwidthTelemetryManager(IEntityComponentSource src, IEntity ent, IEntity parent)
	{
		m_sInstance = this;
		m_fLastSummaryTime = System.GetTickCount();
	}
	...
	//------------------------------------------------------------------------------------------------
	void ~COA_BandwidthTelemetryManager()
	{
		if (m_sInstance == this)
			m_sInstance = null;
	}
	...
	//------------------------------------------------------------------------------------------------
	static COA_BandwidthTelemetryManager GetInstance()
	{
		return m_sInstance;
	}
}
