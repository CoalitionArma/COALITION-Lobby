class COA_MenuManagerClass : ScriptComponentClass {}

class COA_MenuManager : ScriptComponent
{	
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	void RequestToJoinChannel(int channel, int requestId)
	{
		...
		// The logic has been moved to COA_PlayerRplToAuthorityManager.RpcAsk_RequestToJoinChannel
	...
	void Accept()
	{
		COA_ListBoxElementComponent comp = GetComponentFromWidgetHierarchy();
		if (!comp)
			return;
			
		// Join the requester to the channel
		COA_PlayerRplToAuthorityManager.GetInstance().JoinChannel(comp.m_iPlayerId, comp.m_iChannelId);
		
		// Send acceptance notification with sound to the requester
		COA_RplBroadcastManager.GetInstance().NotifyRequestAccepted(comp.m_iPlayerId);
	}
	...
	void Deny()
	{
		...
		COA_RplBroadcastManager.GetInstance().NotifyRequestDenied(comp.m_iPlayerId);
		...
		foreach (int player : players)
		{
			...
			{
				COA_RplBroadcastManager.GetInstance().Deny(player, comp.m_iPlayerId);
			}
		}
	}
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	//------------------------------------------------------------------------------------------------
	private COA_ListBoxElementComponent GetComponentFromWidgetHierarchy()
	{
		...
		return COA_ListBoxElementComponent.Cast(parent.FindHandler(COA_ListBoxElementComponent));
	}
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	//------------------------------------------------------------------------------------------------
	protected static COA_MenuManager m_sInstance;
	void COA_MenuManager(IEntityComponentSource src, IEntity ent, IEntity parent)
	{
		m_sInstance = this;
	}

	//------------------------------------------------------------------------------------------------
	void ~COA_MenuManager()
	{
		if (m_sInstance == this)
			m_sInstance = null;
	}
	...
	//------------------------------------------------------------------------------------------------
	static COA_MenuManager GetInstance()
	{
		return m_sInstance;
	}
}
