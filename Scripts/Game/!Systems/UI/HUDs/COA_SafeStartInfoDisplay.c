class COA_SafeStartInfoDisplay : SCR_InfoDisplayExtended
{
	//------------------------------------------------------------------------------------------------
	...
	// Manager references
	//------------------------------------------------------------------------------------------------
	protected COA_SafestartManager m_SafestartManager = null;
	protected COA_Gamemode m_Gamemode = null;
		...
		if (!m_bDataLoaded && m_Gamemode)
		{
			...
			if (slottingManager)
			{
				...
				if (playerFaction)
				{
					...
					if (playerEntity && !COA_EntityHelper.IsSpectator(playerEntity))
					{
						LoadMissionData();
						m_bDataLoaded = true;
					}
					...
					if (gearScriptSettings)
					{
						...
						if (gearScriptResource)
						{
							...
							if (gearConfig)
							{
								...
								COA_RoleConfig roleConfig = COA_GearscriptManager.GetRolesConfig().FindRoleConfig(playerRole);
								if (roleConfig)
								{
									// Check for binoculars (leadership/assistant get them)
									COA_ESlotType slotType = roleConfig.m_SlottingType;
									if (slotType == COA_ESlotType.SQUAD_LEADER || 
									    slotType == COA_ESlotType.TEAM_LEADER || 
									    slotType == COA_ESlotType.ASSISTANT)
									{
										ResourceName binocularsPrefab;
										if (slotType == COA_ESlotType.ASSISTANT && gearConfig.m_sAssistantBinocularsPrefab)
								...
								{
									foreach (COA_Inventory_Item invItem : gearConfig.m_DefaultInventoryItems)
									{
								...
								foreach (ref COA_Role_Custom_Gear customGear : gearConfig.m_RolesToSetCustomSettings)
								{
									...
									foreach (COA_Inventory_Item invItem : customGear.m_AdditionalInventoryItems)
									{
										if (invItem && invItem.m_sItemPrefab)
										{
											string itemPath = invItem.m_sItemPrefab;
