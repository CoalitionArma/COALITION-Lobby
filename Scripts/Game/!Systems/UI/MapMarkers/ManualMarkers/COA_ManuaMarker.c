class COA_ManualMarkerClass : GenericEntityClass
{
...
*/
class COA_ManualMarker : GenericEntity
{
	...
	COA_ManualMarkerComponent m_hManualMarkerComponent;
	...
	void CreateMapWidget(MapConfiguration mapConfig)
	{
		...
		m_hManualMarkerComponent = COA_ManualMarkerComponent.Cast(m_wRoot.FindHandler(COA_ManualMarkerComponent));
		m_hManualMarkerComponent.SetImage(m_sImageSet, m_sQuadName);
		m_hManualMarkerComponent.SetImageGlow(m_sImageSetGlow, m_sQuadName);
	...
	void COA_ManualMarker(IEntitySource src, IEntity parent)
	{
		// Enable init event
