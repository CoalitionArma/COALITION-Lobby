modded class SCR_GroupsManagerComponent
{
	//------------------------------------------------------------------------------------------------
	//! \param[in] playerID
	//! \param[in] previousGroupID
	//! \param[in] newGroupID
	//! \return
	override int MovePlayerToGroup(int playerID, int previousGroupID, int newGroupID)
	{
		m_iMovingPlayerToGroupID = newGroupID;
		SCR_AIGroup previousGroup = FindGroup(previousGroupID);
		if (previousGroup)
			previousGroup.RemovePlayer(playerID);
		
		SCR_AIGroup newGroup = FindGroup(newGroupID);
		if (newGroup)
		{
			if (newGroup.IsFull())
			{
				m_iMovingPlayerToGroupID = -1;
				return -1;
			}	
			
			if (COA_SlottingManager.GetInstance())
			{
				int slotID = COA_SlottingManager.GetInstance().GetPlayerSlotID(playerID);
				
				if (slotID != -1)
				{
					RplComponent groupRplComponent = RplComponent.Cast(newGroup.FindComponent(RplComponent));
					if (groupRplComponent)
						COA_SlottingManager.GetInstance().UpdateSlotGroup(slotID, groupRplComponent.Id());
				}
			};
			
			newGroup.AddPlayer(playerID);
			m_iMovingPlayerToGroupID = -1;
			return newGroupID;
		}
		else
		{
			m_iMovingPlayerToGroupID = -1;
			return -1;
		}
	}
}
