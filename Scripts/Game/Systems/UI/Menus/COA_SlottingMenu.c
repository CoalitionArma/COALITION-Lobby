class COA_SlottingMenu: ChimeraMenuBase
{
	...
	protected COA_ListboxComponent m_cSlotListBoxComponent;           // Component for showing available slots
	protected COA_ListboxComponent m_cOrbatListBoxComponent;          // Component for showing organization structure
	
	//---------------------------------------------------------------------
	// Game Systems
	//---------------------------------------------------------------------
	protected COA_Gamemode m_Gamemode;          		// Reference to the current gamemode
	...
	protected int m_iSlotListHoldDirection = 0;  // -1/0/1 - which direction COA_SlottingListNext/Prev is currently held in
	...
	{	
		...
		COA_SlottingManager.GetInstance().GetOnSlotChanged().Insert(UpdateSlotInPlace);
	...
	protected void SetupInputListeners()
	{
		...
		GetGame().GetInputManager().AddActionListener("COA_SlottingFactionPrev", EActionTrigger.DOWN, Action_CycleFactionPrev);

		// Controller Y button / keyboard Tab toggles the admin-only Players/Unslotted tab
		GetGame().GetInputManager().AddActionListener("COA_SlottingToggleTab", EActionTrigger.DOWN, Action_ToggleUnslottedTab);
		...
		GetGame().GetInputManager().AddActionListener("COA_SlottingListNext", EActionTrigger.DOWN, Action_SlotListNext);
		GetGame().GetInputManager().AddActionListener("COA_SlottingListPrev", EActionTrigger.DOWN, Action_SlotListPrev);
	}
	...
	protected void InitializeListComponents()
	{
		...
		m_cOrbatListBoxComponent = COA_ListboxComponent.Cast(OverlayWidget.Cast(m_wRoot.FindAnyWidget("OrbatList")).FindHandler(COA_ListboxComponent));
		m_cUnslotPlayerListBoxComponent = COA_ListboxComponent.Cast(OverlayWidget.Cast(m_wRoot.FindAnyWidget("UnslotPlayerList")).FindHandler(COA_ListboxComponent));
	...
	protected void SetupFactionUIDisplay(string factionKey, out ResourceName iconResource, string frameWidget, string flagWidget, string bgSelectWidget, Color bgColor)
	{
		...
		{
			COA_GearScriptConfig gearConfig = COA_GearScriptConfig.Cast(BaseContainerTools.CreateInstanceFromContainer(
	...
	protected void MoveSlotListFocus(int direction)
	{
		...
		for (int attempts = 0; attempts < itemCount; attempts++)
		{
			...
			COA_ListBoxElementComponent elem = m_cSlotListBoxComponent.GetCRFElementComponent(index);
	 ...
	 */
	protected void FocusSlotListItem(int index, COA_ListBoxElementComponent elem)
	{
	...
	override void OnMenuClose()
	{
		...
		COA_SlottingManager.GetInstance().GetOnSlotChanged().Remove(UpdateSlotInPlace);
		...
		GetGame().GetInputManager().RemoveActionListener("COA_SlottingToggleTab", EActionTrigger.DOWN, Action_ToggleUnslottedTab);
		GetGame().GetInputManager().RemoveActionListener("COA_SlottingListNext", EActionTrigger.DOWN, Action_SlotListNext);
		GetGame().GetInputManager().RemoveActionListener("COA_SlottingListPrev", EActionTrigger.DOWN, Action_SlotListPrev);
	}
	...
	void AdvanceSlottingPhase()
	{
		...
		COA_PlayerRplToAuthorityManager.GetInstance().RequestAdvanceSlottingPhase();
	}
	...
	void UpdateSlots()
	{
		...
		if(COA_SlottingManager.GetInstance().IsPlayerInASlot(m_iSelectedplayerId))
	 ...
	 */
	private void PopulateGroupsAndSlots(array<SCR_AIGroup> groups, map<int, ref COA_SlotData> slotMap)
	{
		bool isAdmin = SCR_Global.IsAdmin(GetGame().GetPlayerController().GetPlayerId());
			...
			{	
				...
				if (slotData.GetSlotCurrentPlayerId() > 0)
				{
					...
					if (COA_CommunityTagManager.GetInstance())
					{
						...
						slotPlayerXp = COA_CommunityTagManager.GetInstance().GetPlayerXp(slotData.GetSlotCurrentPlayerId());
						slotPlayerTrack = COA_CommunityTagManager.GetInstance().GetPlayerRankTrack(slotData.GetSlotCurrentPlayerId());
					}
	 ...
	 */
	private void SetupAdminSlotControls(int slotIndex, COA_SlotData slotData)
	{
	...
	void KickSlot()
	{
		...
		COA_PlayerRplToAuthorityManager.GetInstance().UpdateSlotPlayerID(selectedSlotId, 0);
	}
	...
	{
		// Get selected group
		COA_ListBoxElementComponent selectedElement = m_cSlotListBoxComponent.GetCRFElementComponent(
		...
		RplId groupRplID;
		if (!COA_ReplicationHelper.GetRplId(aiGroup, groupRplID))
			return;
		
		// Get all slot IDs for this group
		array<int> slotsInGroup = COA_SlottingManager.GetInstance().GetAllSlotIDsForGroup(groupRplID);
		
		// If group is currently private (locked), unlock it
		if(aiGroup.IsPrivate())
		{
			COA_PlayerRplToAuthorityManager.GetInstance().UpdateGroupLockedState(groupRplID, false);
			
			// Unlock all slots in group using batched updates
			foreach(int slotId : slotsInGroup)
			{
				// Use batched RPC to unlock slot
				COA_PlayerRplToAuthorityManager.GetInstance().UpdateSlotLockedState(slotId, false);
			}
		}
		// If group is currently unlocked, lock it
		else
		{
			COA_PlayerRplToAuthorityManager.GetInstance().UpdateGroupLockedState(groupRplID, true);
			
			// Lock all slots in group using batched updates
			foreach(int slotId : slotsInGroup)
			{
				// Use batched RPC: lock slot and remove player in one call
				COA_PlayerRplToAuthorityManager.GetInstance().UpdateSlotLockedState(slotId, true);
			}
		}
	}
	...
	void LockSlot()
	{
		...
		COA_ListBoxElementComponent selectedComponent = m_cSlotListBoxComponent.GetCRFElementComponent(
			m_cSlotListBoxComponent.GetSelectedItem());
		...
		if(isCurrentlyLocked)
			// Use batched RPC to unlock slot
			COA_PlayerRplToAuthorityManager.GetInstance().UpdateSlotLockedState(selectedSlotId, false);
		else
			// Use batched RPC to lock slot
			COA_PlayerRplToAuthorityManager.GetInstance().UpdateSlotLockedState(selectedSlotId, true);
	}
	...
	void AdvanceMenu()
	{
		COA_PlayerRplToAuthorityManager.GetInstance().RequestAdvanceGamemodeState(false);
	}
	...
	/**
	 * Polled every frame so holding COA_SlottingListNext/Prev keeps stepping through the slot
	...
	protected void UpdateSlotListRepeat(float tDelta)
	{
		...
		float nextVal = im.GetActionValue("COA_SlottingListNext");
		float prevVal = im.GetActionValue("COA_SlottingListPrev");
	...
	private void SetPlayerStatusColor(int playerId, SCR_ListBoxElementComponent comp)
	{
		...
		if (COA_PermissionManager.GetInstance().IsModerator(playerId))
		{
			comp.SetColor(Color.Yellow);
			return;
		}
		
		if (COA_PermissionManager.GetInstance().IsDonator(playerId))
		{
	...
	void SelectSlot()
	{
		...
		COA_ListBoxElementComponent comp = COA_ListBoxElementComponent.Cast(
	 ...
	 */
	private void HandleAdminSlotSelection(int slotId, COA_SlottingManager slottingManager)
	{
		...
		if (currentPlayerId == m_iSelectedplayerId)
		{
			...
			COA_PlayerRplToAuthorityManager.GetInstance().UpdateSlotPlayerID(slotId, 0);
			m_iSelectedplayerId = 0;
		...
		else if (currentPlayerId == 0) 
		{
			...
			if (slottingManager.IsPlayerInASlot(m_iSelectedplayerId))
			{
				...
				COA_PlayerRplToAuthorityManager.GetInstance().UpdateSlotPlayerID(currentSlotId, 0);
			}
			
			// Move player to the new slot
			COA_PlayerRplToAuthorityManager.GetInstance().UpdateSlotPlayerID(slotId, m_iSelectedplayerId);
	 ...
	 */
	private void HandlePlayerSlotSelection(int slotId, COA_SlottingManager slottingManager, int localPlayerId)
	{
		int currentPlayerId = slottingManager.GetSlotData(slotId).GetSlotCurrentPlayerId();
		...
		else if (currentPlayerId == 0) 
		{
			...
			// Move player to the new slot
			COA_PlayerRplToAuthorityManager.GetInstance().UpdateSlotPlayerID(slotId, localPlayerId);
		}
	}
