class COA_SpectatorLabelIconCharacter : COA_SpectatorLabelIcon
{
	...
	protected ResourceName m_rIconImageSet = "{F3A9B47F55BE8D2B}UI/Textures/Icons/Spec_Atlas_x64.imageset";
	...
	protected void UpdatePlayerName()
	{
		...
		bool isDead = !COA_DamageHelper.CheckIfEntityAlive(m_eEntity);

		RplComponent rplComponent = RplComponent.Cast(m_eEntity.FindComponent(RplComponent));
		if (!rplComponent)
			return;
			
		COA_SlotData slotData = COA_SlottingManager.GetInstance().GetSlotDataFromCharacter(rplComponent.Id());
