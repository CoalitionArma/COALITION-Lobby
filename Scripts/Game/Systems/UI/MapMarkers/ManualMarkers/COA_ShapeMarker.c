class COA_ShapeMarkerClass : COA_ManualMarkerClass
{
}

enum COA_EMapShapeMarkerType
{
	CIRCLE,
	SQUARE
}
...
class COA_ShapeMarker : COA_ManualMarker
{
	...
	protected const ResourceName SHAPE_CIRCLE_IMAGESET = "{F3A9B47F55BE8D2B}UI/Images/Atlas/Spec_Atlas_x64.imageset";
	...
	[Attribute("0", UIWidgets.ComboBox, "Shape type", "", ParamEnumArray.FromEnum(COA_EMapShapeMarkerType), category: "Shape Marker")]
	protected COA_EMapShapeMarkerType m_eShapeType;
	...
	protected void UpdateShapeDrawCommands(float centerX, float centerY, float sizeX, float sizeY, float yawDeg)
	{
		...
		if (m_eShapeType == COA_EMapShapeMarkerType.SQUARE)
		{
	...
	protected void ApplyShapeVisual()
	{
		switch (m_eShapeType)
		{
			case COA_EMapShapeMarkerType.SQUARE:
				m_sImageSet = SHAPE_SQUARE_IMAGESET;
				m_sImageSetGlow = "";
				m_sQuadName = SHAPE_SQUARE_QUAD;
				break;

			case COA_EMapShapeMarkerType.CIRCLE:
			default:
