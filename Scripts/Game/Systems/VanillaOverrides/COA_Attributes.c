class COA_BaseContainerCustomTitleResourceFields : BaseContainerCustomTitle
{
	protected ref array<string> m_aPropertyNames;
	protected string m_sFormat;

	//------------------------------------------------------------------------------------------------
	void COA_BaseContainerCustomTitleResourceFields(array<string> propertyNames, string format = "%1")
	{
	...
	override bool _WB_GetCustomTitle(BaseContainer source, out string title)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (source.Get(m_aPropertyNames[i], tempResourceName))
				title = COA_LoggingHelper.SanitizeResourceName(tempResourceName);
