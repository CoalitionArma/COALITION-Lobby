// COA_CVON_GamemodeOverride.c
// Disables the Coalition VON (CVON) system when the mission designer has unchecked "Use CVON"
// on the COA_Lobby entity. When CVON is disabled, the vanilla Arma Reforger in-game VON is
...
modded class CVON_VONGameModeComponent
{
	...
	//! COA_Gamemode that owns this component has elected to use CVON. If not, clear the static
	...
	override void OnPostInit(IEntity owner)
	{
		...
		COA_Gamemode gamemode = COA_Gamemode.Cast(owner);
		if (!gamemode)
			return;
