class COA_SpectatorDamageReportEntry
{
...
class COA_SpectatorDamageReportStore
{
	...
	static void GetEventsForPlayer(int playerId, notnull array<ref COA_SpectatorDamageReportEntry> outEntries)
	{
		outEntries.Clear();
		...
		foreach (COA_SpectatorDamageReportEntry entry : s_aEntries)
		{
...
modded class SCR_DamageManagerComponent
{
	override protected void OnDamage(notnull BaseDamageContext damageContext)
	{
		...
		bool wasAlive = false;
		bool shouldReport = COA_ShouldReportSpectatorDamage(damageContext);

		if (victimEntity && playerManager)
		{
			victimPlayerId = playerManager.GetPlayerIdFromControlledEntity(victimEntity);
			if (victimPlayerId > 0)
				wasAlive = COA_DamageHelper.CheckIfEntityAlive(victimEntity);
		}
		...
		string damageType = COA_DamageHelper.GetDamageTypeString(damageContext.damageType);
		bool fatal = wasAlive && !COA_DamageHelper.CheckIfEntityAlive(victimEntity);
	...
	protected bool COA_ShouldReportSpectatorDamage(notnull BaseDamageContext damageContext)
	{
