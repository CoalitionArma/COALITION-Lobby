...
class COA_ReplicationHelper
{
	...
	static bool ValidateReplicationSetup(IEntity entity, bool printWarnings = true)
	{
		...
		{
			if (printWarnings)
				Print("[COA_ReplicationHelper] WARNING: Null entity passed to ValidateReplicationSetup", LogLevel.WARNING);
			return false;
		}
		...
		if (!rplComp)
		{
			if (printWarnings)
			{
				...
				Print(string.Format("[COA_ReplicationHelper] WARNING: Entity '%1' has no RplComponent but may require replication", entityName), LogLevel.WARNING);
			}
			return false;
		}
		...
		{
			if (printWarnings)
				Print(string.Format("[COA_ReplicationHelper] WARNING: Entity '%1' has RplComponent but invalid RplId", entity.GetName()), LogLevel.WARNING);
			return false;
		}
