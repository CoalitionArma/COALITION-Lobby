class COA_EntityHelper
{	
	// Spectator resources to use
	static const ResourceName SPECTATOR_RESOURCE = "{59886ECB7BBAF5BC}Prefabs/Characters/!Spectator/COA_Spectator.et";
	static const ResourceName SPECTATOR_CAMERA_RESOURCE = "{E1FF38EC8894C5F3}Prefabs/Systems/!Spectator/COA_SpectatorCamera.et";
	...
	static bool IsSpectator(IEntity entity)
	{
		...
		return COA_SpectatorCharacter.Cast(entity);
	}
	...
	static bool IsSpectator()
	{
		...
		if (mainEntity && COA_SpectatorCharacter.Cast(mainEntity))
			return true;
		
		IEntity controlledEntity = SCR_PlayerController.GetLocalControlledEntity();
		if (controlledEntity && COA_SpectatorCharacter.Cast(controlledEntity))
			return true;
