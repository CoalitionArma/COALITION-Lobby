class COA_MissionHelper {

	//------------------------------------------------------------------------------------------------
	static vector GetAOCenter()
	{
		COA_RespawnManager respawnMan = COA_RespawnManager.GetInstance();
		//We are cooked
		if (!respawnMan)
			return "0 0 0";
		...
		{
			array<COA_SpawnPointData> spawnPointDataArray = respawnMan.GetFactionSpawnpoints(factionKey);
			if (spawnPointDataArray && !spawnPointDataArray.IsEmpty())
			{
				COA_SpawnPointData spawnPointData = spawnPointDataArray.Get(0);
