...
{	
	...
	static COA_Weapon_Class SelectRandomWeapon(array<ref COA_Weapon_Class> weaponArray)
	{
		...
		SCR_CharacterInventoryStorageComponent inventory, SCR_InventoryStorageManagerComponent inventoryManager)
	{
		...
		if (!successfulSpawn)
		{
			COA_LoggingHelper.LogItemError(weaponResource, inventoryManager.GetOwner(), "WEAPON");
			return;
		}
	...
	static void SelectWeapon(IEntity entity)
	{
		...
		if (playerId > 0)
		{
			...
			COA_GearscriptCharacter.Cast(entity).SelectPrimaryWeapon();
		}
		else
		{
		...
		EntitySpawnParams spawnParams, SCR_InventoryStorageManagerComponent inventoryManager)
	{
		...
		if (!spawned)
		{
			COA_LoggingHelper.LogItemError(attachmentResource, weapon, "ATTATCHMENT");
		}
	}
	...
	static array<ref COA_Magazine_Class> FindMagArrayForWeaponsSelected(array<COA_Weapon_Class> weaponsSelected, array<ref COA_Weapon_Class> weaponType, out COA_Weapon_Class selectedWeapon)
	{	
		foreach (COA_Weapon_Class weaponSelected : weaponsSelected)
		{
			if (weaponType.Contains(weaponSelected))
			{
				foreach (COA_Weapon_Class weaponToCompare : weaponType)
				{
		...
		return new array<ref COA_Magazine_Class>; 
	}
	...
	static array<ref COA_Magazine_Class> ConvertSpecMagArrayIntoMagArray(array<ref COA_Spec_Magazine_Class> specMagazineArray, bool isAssistant)
	{
		array<ref COA_Magazine_Class> tempArray = {};
		
		if (!specMagazineArray)
			return tempArray;
			
		foreach (COA_Spec_Magazine_Class specMagazine : specMagazineArray)
		{
			if (!specMagazine)
				continue;
				
			ref COA_Magazine_Class tempMag = new COA_Magazine_Class();
