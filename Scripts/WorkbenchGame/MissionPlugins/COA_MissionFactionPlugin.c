...
{		
	...
	{	
		...
		m_BLUFORGearScriptSettings = new COA_SimplifiedGearScriptContainer;
		m_OPFORGearScriptSettings = new COA_SimplifiedGearScriptContainer;
		...
		"This allows you to change basic mission faction settings, for advanced gearscript settings (Mini-Arsenal, Vehicle Gearscript Settings, etc) please go into the COA_Lobby entity's object properties", 
		this))
			return;
	}
	
	//------------------------------------------------------------------------------------------------
	protected void SetPluginGearscriptVariables(COA_SimplifiedGearScriptContainer pluginGearscript, COA_GearScriptContainer gearscript)
	{			
	...
	//------------------------------------------------------------------------------------------------
	protected void SetPluginFriendlyFactions(SCR_FactionManager facManager, FactionKey factionKey, array<COA_EFactions> friendlyFactionArray)
	{
		...
		foreach(FactionKey facKey : friendlyFactions)
		{
			if (facKey == "BLUFOR" || facKey == "OPFOR" || facKey == "INDFOR" || facKey == "CIV")
				friendlyFactionArray.Insert(typename.StringToEnum(COA_EFactions, facKey));
		}
	}
	...
	//------------------------------------------------------------------------------------------------
	protected void SetGamemodeGearscriptVariables(WorldEditorAPI api, IEntitySource entitySource, COA_SimplifiedGearScriptContainer pluginGSContainer, string gearscriptContainterToChange)
	{			
	...
	//------------------------------------------------------------------------------------------------
	protected void SetFactionManagerVariables(WorldEditorAPI api, IEntitySource facManagerSource, FactionKey factionKey, array<COA_EFactions> friendlyFactions)
	{
		SCR_FactionManager facManager = SCR_FactionManager.Cast(api.SourceToEntity(facManagerSource));
		...
		int validCount = 0;
		foreach (COA_EFactions friendlyFactionEnum : friendlyFactions)
		{
			string friendlyFactionStr = SCR_Enum.GetEnumName(COA_EFactions, friendlyFactionEnum);
