...
class COA_MissionSlottingPlugin : WorkbenchPlugin
{	
	//------------------------------------------------------------------------------------------------
	override void Run()
	{
		...
		IEntitySource entitySource = api.FindEntityByName("COA_Lobby"); // Getting the IEntitySource of the lobby, this allows us to set variables on the lobby just like if you were changing Object Properties.
		...
		if (gamemode.m_BluforSlots.IsEmpty() && gamemode.m_OpforSlots.IsEmpty() && gamemode.m_IndforSlots.IsEmpty() && gamemode.m_CivSlots.IsEmpty())
		{
			...
			COA_MissionSlottingSetupDialouge dialog = new COA_MissionSlottingSetupDialouge();
			dialog.Run(); // Have to call Run() Manually since the dialouge isnt a child of WorkbenchPlugin.
...
class COA_MissionSlottingQuickSetupDialouge
{
	...
	static ref array<ResourceName> DEFAULT_SLOTS = {
		"{AA52A4B0CDA99915}Configs/SlottingGroups/Leadership/COA_COY.conf",
		"{F779F56A759256A8}Configs/SlottingGroups/Leadership/COA_PLT.conf",
		"{496BAA2CA4279ADA}Configs/SlottingGroups/Squads/COA_RifleSquad.conf",
		"{496BAA2CA4279ADA}Configs/SlottingGroups/Squads/COA_RifleSquad.conf",
		"{496BAA2CA4279ADA}Configs/SlottingGroups/Squads/COA_RifleSquad.conf",
		"{F779F56A759256A8}Configs/SlottingGroups/Leadership/COA_PLT.conf",
		"{496BAA2CA4279ADA}Configs/SlottingGroups/Squads/COA_RifleSquad.conf",
		"{496BAA2CA4279ADA}Configs/SlottingGroups/Squads/COA_RifleSquad.conf",
		"{496BAA2CA4279ADA}Configs/SlottingGroups/Squads/COA_RifleSquad.conf",
		"{09D9CFEC656E40FE}Configs/SlottingGroups/Specialties/COA_MMG.conf",
		"{3C46FECBC2DC1F33}Configs/SlottingGroups/Specialties/COA_MAT.conf",
		"{C50D6F576810DE04}Configs/SlottingGroups/Crews/COA_AirCrew.conf",
		"{CF658340A75E8ACA}Configs/SlottingGroups/Crews/COA_VehicleCrew.conf"
	};
...
class COA_MissionSlottingSetupDialouge
{
	...
	//------------------------------------------------------------------------------------------------
	protected void SetPluginSlottingVariables(array <ref COA_SlottingGroup> pluginSlots, array <ref COA_SlottingGroup> gamemodeSlots)
	{
	...
	//------------------------------------------------------------------------------------------------
	void SetPluginQuickSlots(array <ResourceName> quickSlots, array <ref COA_SlottingGroup> slots)
	{
		...
		{
			COA_SlottingGroup slotGroup = COA_SlottingGroup.Cast(BaseContainerTools.CreateInstanceFromContainer(BaseContainerTools.LoadContainer(slotConfig).GetResource().ToBaseContainer()));
	...
	//------------------------------------------------------------------------------------------------
	protected void SetGamemodeSlottingVariables(WorldEditorAPI api, IEntitySource entitySource, array <ref COA_SlottingGroup> pluginSlots, string slotsToChange, array <ref COA_SlottingGroup> gamemodeSlots)
	{			
		for (int r; r < gamemodeSlots.Count(); r++)
			api.RemoveObjectArrayVariableMember(entitySource, null, slotsToChange, 0);
		
		foreach (int i, COA_SlottingGroup slotGroup : pluginSlots)
		{
			array<ref ContainerIdPathEntry> path = {ContainerIdPathEntry(slotsToChange, i)};
			
			api.CreateObjectArrayVariableMember(entitySource, null, slotsToChange, "COA_SlottingGroup", i);
			...
			foreach (int f, COA_EGearRole role : slotGroup.m_aSlots)
			{
				if (f == 0)
			...
			foreach (int j, COA_RoleRespawnEntry roleRespawn : slotGroup.m_aRoleRespawns)
			{
				...
				api.CreateObjectArrayVariableMember(entitySource, path, "m_aRoleRespawns", "COA_RoleRespawnEntry", j);
