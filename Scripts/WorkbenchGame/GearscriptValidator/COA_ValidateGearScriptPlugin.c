...
/*
	...
	per-role custom gear - and attempts to load each one the same way COA_GearscriptManager does
...
*/
class COA_ValidateGearScriptPlugin : ResourceManagerPlugin
{
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	protected void ValidateGearScript(ResourceName resourceName)
	{
		...
		{
			foreach (COA_Role_Custom_Gear roleGear : config.m_RolesToSetCustomSettings)
			{
				if (!roleGear)
					continue;
	...
	protected void CheckIdentity(ResourceName identityResource)
	{
		...
		COA_CharacterIdentity identity = COA_CharacterIdentity.Cast(BaseContainerTools.CreateInstanceFromContainer(containerResource.GetResource().ToBaseContainer()));
		if (!identity || !identity.m_VisualIdentityArray)
			return;

		foreach (COA_Character_Visual_Identity visual : identity.m_VisualIdentityArray)
		{
			if (!visual)
				continue;
	...
	//------------------------------------------------------------------------------------------------
	protected void CheckWeaponArray(array<ref COA_Weapon_Class> weapons, string label)
	{
		if (!weapons)
			return;
	...
	//------------------------------------------------------------------------------------------------
	protected void CheckWeapon(COA_Weapon_Class weapon, string label)
	{
		if (!weapon)
			return;
	...
	//------------------------------------------------------------------------------------------------
	protected void CheckSpecWeapon(COA_Spec_Weapon_Class weapon, string label)
	{
		if (!weapon)
			return;
	...
	//------------------------------------------------------------------------------------------------
	protected void CheckClothingArray(array<ref COA_Clothing> clothingArray, string label)
	{
		if (!clothingArray)
			return;
		...
		foreach (COA_Clothing clothing : clothingArray)
		{
			...
			string slotLabel = string.Format("%1 (%2)", label, typename.EnumToString(COA_EGearscriptClothing, clothing.m_iClothingType));
	...
	//------------------------------------------------------------------------------------------------
	protected void CheckInventoryArray(array<ref COA_Inventory_Item> items, string label)
	{
		if (!items)
			return;
	...
	protected bool CheckResource(ResourceName resourceName, string label)
	{
		...
		// COA_GearscriptManager.AddInventoryItem performs at runtime before spawning gear.
